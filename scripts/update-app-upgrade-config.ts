...
async function applyChannelUpdate(
  ...
  if (skipReleaseValidation) {
    ...
    if (releaseInfo.channel === 'latest' && !availability.gitcode) {
      console.warn(
        `[update-app-upgrade-config] gitcode feed is not ready for ${releaseInfo.tag}. Falling back to the self-hosted feed URL.`
      )
...
function getReleasePageUrl(mirror: UpdateMirror, tag: string): string {
  ...
  if (mirror === 'github') {
    return `${SELF_HOSTED_RELEASE_BASE_URL}/${encodedTag}/latest.yml`
  }
  return `${SELF_HOSTED_RELEASE_BASE_URL}/${encodedTag}/latest.yml`
}
