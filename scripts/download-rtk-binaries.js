...
function hasMatchingLocalRtk(platformKey, outputDir) {
  const pkg = RTK_PACKAGES[platformKey]
  if (!pkg) {
    return false
  }

  const binaryPath = path.join(outputDir, pkg.binary)
  const versionPath = path.join(outputDir, '.rtk-version')
...
function main() {
  ...
  if (hasMatchingLocalRtk(platformKey, outputDir)) {
    console.log(`[rtk] Found local rtk ${RTK_VERSION} for ${platformKey}, skipping download`)
    return
  }
