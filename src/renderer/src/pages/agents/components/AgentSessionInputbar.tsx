...
const AgentSessionInputbarInner: FC<InnerProps> = ({ assistant, agentId, sessionId, sessionData, actionsRef }) => {
  ...
  const hasModel = Boolean(assistant.model)
  const sendDisabled = (inputEmpty && files.length === 0) || !apiServer.enabled || !hasModel
  ...
  const sendMessage = useCallback(async () => {
    if (!assistant.model) {
      window.toast.error(t('agent.input.model_required'))
      return
    }
  ...
  const placeholderText = useMemo(() => {
    if (!assistant.model) {
      return t('agent.input.model_required_placeholder')
    }
  ...
  }, [agentBase?.configuration, assistant.model, sendMessageShortcut, t])
