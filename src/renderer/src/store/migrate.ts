...
function normalizePentimeSidebarIcons(state: RootState) {
  if (!state.settings?.sidebarIcons) {
    return
  }

  const replaceMinApp = (icons: string[]) => icons.map((icon) => (icon === 'minapp' ? 'video' : icon))
  const unique = (icons: string[]) => Array.from(new Set(icons))
  const originalVisible = state.settings.sidebarIcons.visible
  const originalDisabled = state.settings.sidebarIcons.disabled
  const normalizedVisible = unique(replaceMinApp(originalVisible))
  const normalizedDisabled = unique(replaceMinApp(originalDisabled)).filter((icon) => !normalizedVisible.includes(icon))

  if (!normalizedVisible.includes('video') && !normalizedDisabled.includes('video')) {
    normalizedVisible.push('video')
  }

  const defaultOrder = DEFAULT_SIDEBAR_ICONS as string[]
  const visibleSet = new Set(normalizedVisible)
  const orderedVisible = [
    ...defaultOrder.filter((icon) => visibleSet.has(icon)),
    ...normalizedVisible.filter((icon) => !defaultOrder.includes(icon) && icon !== 'minapp')
  ]
  const orderedVisibleSet = new Set(orderedVisible)

  state.settings.sidebarIcons.visible = orderedVisible as RootState['settings']['sidebarIcons']['visible']
  state.settings.sidebarIcons.disabled = normalizedDisabled.filter(
    (icon) => icon !== 'minapp' && !orderedVisibleSet.has(icon)
  ) as RootState['settings']['sidebarIcons']['disabled']
}
...
const migrateConfig = {
    ...
    try {
      ...
      state.assistants.assistants.forEach((assistant) => {
        ...
        if (assistant.defaultModel?.provider === 'cherryai' && assistant.defaultModel?.id === GLM_4_5_FLASH_MODEL) {
          assistant.defaultModel = undefined
        }
      })
  ...
  '209': (state: RootState) => {
    try {
      normalizePentimeSidebarIcons(state)
