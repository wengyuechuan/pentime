...
export function getDefaultModel() {
  const { defaultModel, providers } = store.getState().llm
  if (defaultModel && defaultModel.provider !== 'cherryai') {
    return defaultModel
  }

  const fallbackModel =
    providers
      .filter((provider) => provider.enabled && provider.id !== 'cherryai')
      .flatMap((provider) => provider.models.map((model) => ({ ...model, provider: model.provider || provider.id })))
      .find((model) => model.name === 'Qwen3-8B' || model.id.includes('Qwen3-8B')) ??
    providers
      .filter((provider) => provider.enabled && provider.id !== 'cherryai')
      .flatMap((provider) => provider.models.map((model) => ({ ...model, provider: model.provider || provider.id })))[0]
