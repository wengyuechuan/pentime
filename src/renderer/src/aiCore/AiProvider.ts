...
const normalizeEditImageInput = (input: EditImageParams['inputImages'][number]) => {
...
export default class AiProvider {
  ...
  private async modernEditImage(params: EditImageParams, providerConfig: ProviderConfig): Promise<string[]> {
    ...
    const normalizedInputImages = inputImages.map(normalizeEditImageInput)
    const normalizedMask = mask ? normalizeEditImageInput(mask) : undefined
    ...
    const result = await executor.generateImage({
      ...
      prompt: {
        text: prompt,
        images: normalizedInputImages, // 输入图像（必需）
        ...(normalizedMask && { mask: normalizedMask }) // 可选的 mask（用于 inpainting）
      },
      size: (imageSize || '1024x1024') as `${number}x${number}`,
      ...(signal && { abortSignal: signal })
    })
